...
#[test]
fn test_bzip2_compress_single_file() {
    ...
    let compressed_file = temp_dir.path().join("test.txt.bz2");
    assert!(
...
#[test]
fn test_bzip2_compress_multiple_files() {
    let temp_dir = TempDir::new().unwrap();
    ...
    assert!(file_exists(&temp_dir.path().join("file1.txt.bz2")));
    assert!(file_exists(&temp_dir.path().join("file2.txt.bz2")));
...
#[test]
fn test_bzip2_compress_with_level_1() {
    let temp_dir = TempDir::new().unwrap();
...
#[test]
fn test_bzip2_compress_with_level_9() {
    let temp_dir = TempDir::new().unwrap();
    let test_file = create_test_file(temp_dir.path(), "test.txt", TEST_DATA_MEDIUM);
...
#[test]
fn test_bzip2_compress_with_default_level() {
    let temp_dir = TempDir::new().unwrap();
    let test_file = create_test_file(temp_dir.path(), "test.txt", TEST_DATA_MEDIUM);
...
#[test]
fn test_bzip2_decompress_single_file() {
    let temp_dir = TempDir::new().unwrap();
    let test_file = create_test_file(temp_dir.path(), "test.txt", TEST_DATA_SMALL);
...
#[test]
fn test_bzip2_decompress_multiple_files() {
    let temp_dir = TempDir::new().unwrap();
    let files = create_test_files(
        temp_dir.path(),
        &[
    ...
    let bz1 = temp_dir.path().join("file1.txt.bz2");
    let bz2 = temp_dir.path().join("file2.txt.bz2");
...
#[test]
fn test_bzip2_compress_binary_data() {
    let temp_dir = TempDir::new().unwrap();
    ...
    let compressed_file = temp_dir.path().join("binary.dat.bz2");
    assert!(file_exists(&compressed_file));
...
#[test]
fn test_bzip2_verify_compression_reduces_size() {
    let temp_dir = TempDir::new().unwrap();

    // Create a highly compressible file (repeated content)
    let compressible_content = "B".repeat(10000);
...
#[test]
fn test_bzip2_compress_preserves_original() {
    let temp_dir = TempDir::new().unwrap();
    let test_file = create_test_file(temp_dir.path(), "test.txt", TEST_DATA_SMALL);
